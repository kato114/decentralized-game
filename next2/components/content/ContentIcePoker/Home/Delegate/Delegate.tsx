...
const IceDashboardDelegate = (): ReactElement => {
  ...
  const [accountAgeMultiplier] = useState(9);
    ...
    <section className={cn('row', styles.main_wrapper)}>
      <div className={cn('col-xl-8', styles.overview_container)}>
        ...
        <div className={styles.second_container}>
          <div className={cn(styles.box, styles.get_wearable)}>
            <h1 className={styles.title}>Get a Wearable</h1>
            <img src="https://res.cloudinary.com/dnzambf4m/image/upload/v1645798923/image_44_3x_dg1sp4.png" alt="wearables" />
            <p>Mint wearables for a discount or buy directly on secondary markets.</p>
            <Button
          ...
          <div className={cn(styles.box, styles.player_leaderboard)}>
            <h1 className={styles.title}>Player Leaderboard</h1>
            <div className={styles.players_box}>
              {gameRecords && gameRecords.length > 0
                ? gameRecords.map((gameRecord, i) => {
                    if (i < 3) {
                      return (
                        <div className={styles.player} key={i}>
                          <div className={styles.number}>{i + 1}</div>
                          <div className={styles.avatar}>
                            <img src={`https://events.decentraland.org/api/profile/${gameRecord.address}/face.png`} alt="avatar" />
                          </div>
                          <div className={styles.detail}>
                            <div className={styles.address}>
                              {gameRecord.address.substr(0, 5) + '...' + gameRecord.address.substr(gameRecord.address.length - 4, gameRecord.address.length)}
                            </div>
                            <div className={styles.chip}>
                              {(Number(gameRecord.winnings) / 1000000000000000000).toFixed(0).toLocaleString()}
                              <img src="https://res.cloudinary.com/dnzambf4m/image/upload/v1645037931/Chip_n5cvkl.png" alt="chips" />
                            </div>
                          </div>
              ...
              {gameRecords.length < 2
                ? emptyRecords.map((empty, i) => {
                    if (i < 3 - gameRecords.length) {
                      return (
                        <div className={styles.player} key={i}>
                          <div className={styles.number}>{gameRecords.length + i + 1}</div>
                          <div className={styles.avatar}>
              ...
              <div className={cn(styles.player, styles.active)}>
                <div className={styles.number}>{personalRecord.myRank}</div>
                <div className={styles.avatar}>
                  <img src={`https://events.decentraland.org/api/profile/${state.userAddress}/face.png`} alt="avatar" />
                </div>
                <div className={styles.detail}>
                  <div className={styles.address}>{state.userInfo.name === null || state.userInfo.name === '' ? 'Unnamed' : state.userInfo.name}</div>
                  <div className={styles.chip}>
                    {(Number(personalRecord.myScore) / 1000000000000000000).toFixed(0).toLocaleString()}
                    <img src="https://res.cloudinary.com/dnzambf4m/image/upload/v1645037931/Chip_n5cvkl.png" alt="chips" />
                  </div>
                </div>
                <div className={styles.more}>
      ...
      <div className={cn('col-xl-4', styles.sub_profile)}>
        ...
        {/* <div className={cn(styles.lower)}>
          <p className={styles.lower_header}>Account Age Bonus</p>
          <div className={styles.account_age_bonus}>
            <div className={styles.content}>
              <div className={styles.box} style={{ marginLeft: `${accountAgeMultiplier * 14.4}px` }}>
          ...
          <p className={styles.lower_text}>
            Your account age multiplier <br />
            increments up with daily check-ins. <br />
            Check in 10 days to get max rewards.
          </p>
        </div> */}
...
export default IceDashboardDelegate;
