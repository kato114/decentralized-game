...
const ModalIceDelegationBreakDown = ({ iceWearableItems, playerAddress, delegationBreakdown, setShowingBreakDown }) => {
  const [state, dispatch] = useContext(GlobalContext);
  const [delegatedItem, setDelegatedItem] = useState(null);
  const [copied, setCopied] = useState(false);

  // helper functions
  const onCopy = playerAddress => {
  ...
  useEffect(() => {
    let index = iceWearableItems.findIndex(e => e.delegationStatus.delegatedTo && e.delegationStatus.delegatedTo === playerAddress);
    setDelegatedItem(iceWearableItems[index]);
  }, [iceWearableItems, playerAddress]);
      ...
      >
        ...
        <div className={styles.body}>
          <div className={styles.title}>
            <div className={cn('account-copy', styles.delegate_address)} onClick={() => onCopy(playerAddress)}>
              {state.userAddress ? (
          ...
          <div className={styles.history}>
            <div className={styles.header}>
              ...
              <div className={styles.netChipsScore}>Net Chips Score</div>
              ...
              {delegationBreakdown.map((breakdown, index) => {
                return (
                  <div key={index} className={styles.row}>
                    <div className={styles.address}>{moment.utc(breakdown.gameplayDay).format('MM/DD/YY')}</div>
                    ...
                    <div className={styles.netChipsScore}>
                      {breakdown.chipsWon ? breakdown.chipsWon : '- -'}
                    ...
                    <div className={styles.tier} style={{ paddingLeft: '65px', textAlign: 'left' }}>
                      ...
                      {breakdown.leaderboardPercentile + 5 <= 50 ? `Top ${breakdown.leaderboardPercentile + 5}%` : `Bottom ${100 - breakdown.leaderboardPercentile}%`}
                    </div>
                  </div>
                );
              })}
