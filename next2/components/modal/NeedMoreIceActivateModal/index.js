...
import styles from './NeedMoreIceActivateModal.module.scss';

const NeedMoreIceActivateModal = props => {
  // get user's status from the Context API store
  const [state, dispatch] = useContext(GlobalContext);
  const [open, setOpen] = useState(false);

  console.log('########', state.tokenAmounts.ICE_MOVE_AMOUNT);
  ...
  return (
    <>
      <Modal
        ...
        open={open}
        close
        trigger={<Button className={styles.open_button}>Activate Wearable ({state.tokenAmounts.ICE_MOVE_AMOUNT} ICE)</Button>}
      >
        ...
        <div className={styles.title}>Need More ICE to Activate</div>
        <div className={styles.desc}>In order to earn ICE, you must first activate your wearable. ICE Wearables are activated on the Polygon sidechain.</div>

        <div className={styles.logo}>
          <img src="https://res.cloudinary.com/dnzambf4m/image/upload/c_scale,w_210,q_auto:good/v1631324990/ICE_Diamond_ICN_kxkaqj.svg" alt="logo" />
          <div className={styles.round_desc}>Need {(state.tokenAmounts.ICE_MOVE_AMOUNT - state.iceAmounts?.ICE_AVAILABLE_AMOUNT).toFixed(2)} more</div>
        </div>
        ...
        <div className={styles.buttons}>
          ...
          <div className={styles.button}>
            ...
            <span className={styles.text}>{parseFloat(state.iceAmounts?.ICE_AVAILABLE_AMOUNT).toFixed(2)} ICE Available</span>
          </div>
        </div>
      </Modal>
    </>
...
export default NeedMoreIceActivateModal;
