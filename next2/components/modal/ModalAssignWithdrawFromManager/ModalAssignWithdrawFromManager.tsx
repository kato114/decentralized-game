...
import styles from './ModalAssignWithdrawFromManager.module.scss';
import cn from 'classnames';
import Fetch from 'common/Fetch';

export interface ModalAssignWithdrawFromManagerType {
  address: string;
  setAssignModalShow: any;
}

const ModalAssignWithdrawFromManager: FC<ModalAssignWithdrawFromManagerType> = ({ address, setAssignModalShow }: ModalAssignWithdrawFromManagerType): ReactElement => {
  // fetch user's wallet address from the Context API store
  const [state, dispatch] = useContext(GlobalContext);
  const [isClicked, setIsClicked] = useState(false);

  const withdrawFromManagerOnClick = async (): Promise<null> => {
    setIsClicked(true);
    ...
    <Modal className={cn(styles.assign_withdraw_manager)} onClose={() => setAssignModalShow(false)} open={true} close>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.description}>
          Assigned manager&apos;s address: <a>{address.substr(0, 5) + '...' + address.substr(address.length - 4, address.length)}</a>
        </div>
...
export default ModalAssignWithdrawFromManager;
